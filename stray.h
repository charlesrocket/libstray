...
// public API
TrayIcon* systray_create(const char *app_name, const char *icon_name, const char *title);
void systray_set_click_callback(TrayIcon *icon, TrayClickCallback callback, void *user_data);
...
static void add_string_property(DBusMessageIter *array, const char *prop_name, const char *value) {
...
static void add_empty_pixmap_array(DBusMessageIter *variant) {
    ...
    dbus_message_iter_open_container(variant, DBUS_TYPE_ARRAY, "(iiay)", &pixmap_array);
...
static void emit_signal(TrayIcon *icon, const char *signal_name) {
    ...
    DBusMessage *msg = dbus_message_new_signal(STRAY_OBJECT_PATH, STRAY_INTERFACE_NAME, signal_name);
...
static void emit_properties_changed(TrayIcon *icon, const char *property_name) {
    ...
    DBusMessage *msg = dbus_message_new_signal(STRAY_OBJECT_PATH, 
                                               "org.freedesktop.DBus.Properties", 
                                               "PropertiesChanged");
    if (!msg) return;

    DBusMessageIter args, changed_props, invalidated_props;
    const char *interface = STRAY_INTERFACE_NAME;

    dbus_message_iter_init_append(msg, &args);
    dbus_message_iter_append_basic(&args, DBUS_TYPE_STRING, &interface);

    // build changed properties array
    dbus_message_iter_open_container(&args, DBUS_TYPE_ARRAY, "{sv}", &changed_props);
    
    const char *current_icon = icon->icon_name ? icon->icon_name : STRAY_DEFAULT_ICON;
    const char *current_title = icon->title ? icon->title : STRAY_DEFAULT_TITLE;

    if (strcmp(property_name, "IconName") == 0 || strcmp(property_name, "All") == 0) {
        add_string_property(&changed_props, "IconName", current_icon);
    }
    
    if (strcmp(property_name, "Title") == 0 || strcmp(property_name, "All") == 0) {
    ...
    dbus_message_iter_open_container(&args, DBUS_TYPE_ARRAY, "s", &invalidated_props);
...
static void handle_property_get_all(DBusConnection *conn, DBusMessage *msg, TrayIcon *icon) {
    ...
    DBusMessageIter dict_entry, variant;
    const char *prop_pixmap = "IconPixmap";
    dbus_message_iter_open_container(&array, DBUS_TYPE_DICT_ENTRY, NULL, &dict_entry);
    dbus_message_iter_append_basic(&dict_entry, DBUS_TYPE_STRING, &prop_pixmap);
    dbus_message_iter_open_container(&dict_entry, DBUS_TYPE_VARIANT, "a(iiay)", &variant);
    add_empty_pixmap_array(&variant);
    dbus_message_iter_close_container(&dict_entry, &variant);
    dbus_message_iter_close_container(&array, &dict_entry);
    
    // add Menu property
    const char *prop_menu = "Menu";
    dbus_message_iter_open_container(&array, DBUS_TYPE_DICT_ENTRY, NULL, &dict_entry);
    dbus_message_iter_append_basic(&dict_entry, DBUS_TYPE_STRING, &prop_menu);
    dbus_message_iter_open_container(&dict_entry, DBUS_TYPE_VARIANT, "o", &variant);
    const char *menu_path = "/NO_DBUSMENU";
    dbus_message_iter_append_basic(&variant, DBUS_TYPE_OBJECT_PATH, &menu_path);
    dbus_message_iter_close_container(&dict_entry, &variant);
    dbus_message_iter_close_container(&array, &dict_entry);
    
    // add ItemIsMenu property
    const char *prop_item_is_menu = "ItemIsMenu";
    dbus_message_iter_open_container(&array, DBUS_TYPE_DICT_ENTRY, NULL, &dict_entry);
    dbus_message_iter_append_basic(&dict_entry, DBUS_TYPE_STRING, &prop_item_is_menu);
...
static void handle_property_get(DBusConnection *conn, DBusMessage *msg, TrayIcon *icon, const char *prop) {
    DBusMessage *reply = dbus_message_new_method_return(msg);
    if (!reply) return;
    ...
    if (strcmp(prop, "Category") == 0) {
        ...
        DBusMessageIter variant;
        dbus_message_iter_open_container(&args, DBUS_TYPE_VARIANT, "a(iiay)", &variant);
        add_empty_pixmap_array(&variant);
        dbus_message_iter_close_container(&args, &variant);
    } else if (strcmp(prop, "Menu") == 0) {
        DBusMessageIter variant;
        dbus_message_iter_open_container(&args, DBUS_TYPE_VARIANT, "o", &variant);
        const char *menu_path = "/NO_DBUSMENU";
        dbus_message_iter_append_basic(&variant, DBUS_TYPE_OBJECT_PATH, &menu_path);
        dbus_message_iter_close_container(&args, &variant);
        ...
        DBusMessage *error = dbus_message_new_error(msg,
            "org.freedesktop.DBus.Error.InvalidArgs", "Property not found");
...
static DBusHandlerResult message_handler(DBusConnection *conn, DBusMessage *msg, void *data) {
    ...
    if (strcmp(interface, STRAY_INTERFACE_NAME) == 0 && strcmp(member, "Activate") == 0) {
    ...
    if (strcmp(interface, STRAY_INTERFACE_NAME) == 0 && strcmp(member, "NewIcon") == 0) {
        DBusMessage *reply = dbus_message_new_method_return(msg);
        if (reply) {
            dbus_connection_send(conn, reply, NULL);
            dbus_message_unref(reply);
...
static int register_with_watcher(DBusConnection *conn, const char *service_name) {
    DBusMessage *msg = dbus_message_new_method_call(
        STRAY_WATCHER_SERVICE, STRAY_WATCHER_PATH, STRAY_WATCHER_SERVICE, "RegisterStatusNotifierItem");
    
    if (!msg) return 0;
    
    const char *item_path = STRAY_OBJECT_PATH;
    dbus_message_append_args(msg, DBUS_TYPE_STRING, &item_path, DBUS_TYPE_INVALID);
...
static void process_events_with_timeout(DBusConnection *conn, int timeout_ms) {
    ...
    while (1) {
        ...
        // if we processed something, continue for a bit more to catch any follow-up messages
...
TrayIcon* systray_create(const char *app_name, const char *icon_name, const char *title) {
    ...
    snprintf(service_name, sizeof(service_name), "org.kde.StatusNotifierItem-%d-%s", getpid(), app_name);
    ...
    TrayIcon *icon = calloc(1, sizeof(TrayIcon));
    ...
    if (!dbus_connection_register_object_path(conn, STRAY_OBJECT_PATH, &vtable, icon)) {
        goto cleanup_full;
    }
